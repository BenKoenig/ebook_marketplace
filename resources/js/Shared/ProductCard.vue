<template>
    <div class="relative grid grid-cols-2 sm:grid-cols-1 xl:grid-cols-2 gap-x-2 md:gap-x-3 items-center bg-white rounded-lg border border-black max-w-[45rem] overflow-hidden">
        <!-- Book cover-->
        ...
        <div class="pr-2 sm:p-2 xl:pl-0 xl:pb-0 xl:pt-0">
            ...
            <div v-if="$page.props.auth.user ? $page.props.auth.user.id !== product.user_id : true" class="pb-2 hidden sm:block">
                <p class="text-md tracking-wider text-gray-500 leading-3">By</p>
            ...
            <CustomLink
                :href="'/e/' + product.slug"
                text="Read about it"
                class="w-full bg-yellow-100 hover:bg-yellow-50"
            />
            ...
            <CustomLink
                ...
                text="Edit"
                class="w-full mt-2 bg-white hover:bg-gray-100"
            />
        </div>
    </div>
</template>
